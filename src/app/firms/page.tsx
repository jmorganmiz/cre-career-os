...
import { Edit3, Trash2, Upload, UsersRound } from "lucide-react";
...
export default function FirmsPage() {
  ...
  const closeModal = () => {
    setOpen(false);
    if (window.location.hash === "#new") window.history.replaceState(null, "", "/firms");
  };

  const submit = async (form: FormData) => { await add("firms", Object.fromEntries(form) as Firm); closeModal(); };
  ...
  {live && !firms.length && !query && <div className="mb-5"><EmptyState title="No firms yet" text="Supabase is connected and ready. Add your first target firm to start building the CRM." action="Add first firm" onAction={() => setOpen(true)} /></div>}
