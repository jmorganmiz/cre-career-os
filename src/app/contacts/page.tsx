...
export default function ContactsPage() {
  ...
  <div className="card overflow-hidden"><TableToolbar placeholder="Search by name, firm, or title..." /><div className="overflow-x-auto"><table className="w-full min-w-[850px] text-left text-sm"><thead className="bg-[#f8faf9] text-[10px] uppercase tracking-[.12em] text-[#7a8781]"><tr>{["Contact","Firm","Status","Follow up","Relationship",""].map(h=><th key={h} className="px-5 py-3 font-extrabold">{h}</th>)}</tr></thead><tbody className="divide-y divide-[#edf0ee]">{contacts.map((c,i)=><tr key={c.id} className="hover:bg-[#fafbfa]"><td className="px-5 py-4"><div className="flex items-center gap-3"><div className="grid h-9 w-9 place-items-center rounded-full bg-[#d9efe7] text-xs font-extrabold text-[#164c3a]">{c.first_name[0]}{c.last_name[0]}</div><div><div className="font-extrabold">{c.first_name} {c.last_name}</div><div className="mt-0.5 text-xs text-[#7a8781]">{c.title || "Title TBD"}</div></div></div></td><td className="px-5 py-4 font-bold">{firms.find(f=>f.id===c.firm_id)?.name || "No firm linked"}</td><td className="px-5 py-4"><Badge tone={i===0?"lime":"green"}>{c.status || "Target"}</Badge></td><td className="px-5 py-4 text-[#68766f]">{c.follow_up_at?.slice(0,10) || "Not set"}</td><td className="px-5 py-4"><div className="flex items-center gap-3"><div className="h-1.5 w-20 rounded-full bg-[#edf1ef]"><div className="h-full rounded-full bg-[#4f806b]" style={{width:`${c.relationship_score || 0}%`}} /></div><span className="text-xs font-extrabold">{c.relationship_score || 0}</span></div></td><td className="px-5 py-4"><div className="flex gap-3 text-[#718079]"><Mail size={16}/><button onClick={()=>remove("contacts",c.id)}><Trash2 size={16}/></button></div></td></tr>)}</tbody></table></div></div>
