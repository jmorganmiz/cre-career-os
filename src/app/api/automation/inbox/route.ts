...
export async function GET() {
  ...
  try {
    const admin = db();
    const { data, error } = await admin.from("automation_results").select("*").eq("user_id", auth.user.id).order("created_at", { ascending: false }).limit(100);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(await handleAutomationInboxAction(admin, auth.user.id, body.id, body.action));
  } catch (error) {
